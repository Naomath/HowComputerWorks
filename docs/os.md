...
例えば、ライブラリに含まれる``printf``関数は``printf("Number %d", number)``とすることで、変数``number``の値が表示されるようになるなど、便利な機能がいろいろあります。
これを毎回自前で作成していると大変なので、他のプロセスからも参照できるように開発環境にダウンロードしておきます。
そして、それを参照するようにコードには書くだけで、その開発環境では、必要なコードを事前にダウンロードしていたファイルからコピーして、他のコンピューターに移しても実行可能なバイナリを作成することができます。
というのが静的リンクの仕組みです。
静的 static　があるということは、動的 dynamic　もあります。

実際のリンクはほとんどが動的リンクです。
動的リンクでは、ライブラリに含まれるコードを実行ファイルにそのまま埋め込むのではなく、「このライブラリのこの関数を使いますよ」という参照だけを埋め込んでおきます。
実際のコード本体はライブラリファイル（例えば libc.so.6）に置いたままで、プログラムが実行されるときに必要なライブラリを探して読み込み、関数を結びつけるのです。

動的リンクの利点は、まず実行ファイルが小さくなることです。同じライブラリを複数のプログラムで共有できるので、メモリの節約にもつながります。また、ライブラリを更新すればプログラムを作り直さなくても最新の修正や機能を利用できるという点も大きなメリットです。
一方で、実行環境に必要なライブラリが存在しないと動かせない、あるいはライブラリのバージョン違いによって不具合が起きるといったデメリットもあります。

このように、静的リンクは「すべてを自前に抱え込む」方法で、動的リンクは「必要なものを実行時に持ってくる」方法だと整理すると分かりやすいでしょう。
...
また、以下のコマンドで静的リンクをさせることができます。
```bash
gcc -static hello.c -o hello_static.out
```
これも``objdump -d hello_static.out``を実行するともはや出力が止まらないでしょう。
Ctrl+Cで終わらせてください。
``ls -l``でバイト数が5列目に表示されますが、``hello.out``と``hello_static.out``でファイルサイズを比べてみてください。